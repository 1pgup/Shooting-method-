...
%Finding eigenfunctions and eigenvalues of the Hamiltonian
[eigenvectors, eigenvalues] = eig(H);

%First energy level
psi_fdm_1 = -eigenvectors(:, 1)' / sqrt(L / (N + 1));
%Second energy level
psi_fdm_2 = -eigenvectors(:, 2)' / sqrt(L / (N + 1));
%Third energy level
psi_fdm_3 = -eigenvectors(:, 3)'/ sqrt(L / (N + 1));


%Filling cell array with analytically calculated energies
C(3, 2) = num2cell(eigenvalues(1, 1) * J2eV * 1e9 ^ 2);
C(3, 3) = num2cell(eigenvalues(2, 2) * J2eV * 1e9 ^ 2);
C(3, 4) = num2cell(eigenvalues(3, 3) * J2eV * 1e9 ^ 2);

%Dependence of the wave function on the coordinate
subplot(2, 3, 2);

%First energy level
plot(xp, psi_fdm_1 , 'LineWidth', 2);
hold on;

%Second energy level
plot(xp, psi_fdm_2 , 'LineWidth', 2);

%Third energy level
plot(xp, psi_fdm_3 , 'LineWidth', 2);
...
%First energy level
plot(xp, abs(psi_fdm_1) .^2, 'LineWidth', 2);
hold on;

%Second energy level
plot(xp, abs(psi_fdm_2) .^2, 'LineWidth', 2);

%Third energy level
plot(xp, abs(psi_fdm_3) .^2, 'LineWidth', 2);
...
%psi_sh_1 and psi_fdm_1
plot(xp, abs(psi_fdm_1 - psi_sh_1), 'LineWidth', 2);
hold on;
%psi_sh_2 and psi_fdm_2
plot(xp, abs(psi_fdm_2 - psi_sh_2), 'LineWidth', 2);

%Graphics customization
title('Difference');
legend('|\psi_{fdm}^{n = 1} - \psi_{shoot}^{n = 1}|', ...
    '|\psi_{fdm}^{n = 2} - \psi_{shoot}^{n = 2}|', ...
    'Location', 'northwest');
grid on;
xlabel('x, nm');
...
plot(xp, abs(psi_fdm_3 - psi_sh_3), 'LineWidth', 2);
hold on;
...
grid on;
xlabel('x, nm');
ylabel('|\psi_{fdm}^{n = 3} - \psi_{shoot}^{n = 3}|, nm^{-0.5}');
xlim([0, L]);
sgtitle('Electron in 1d-PW');
